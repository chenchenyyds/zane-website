...
export default function EditProject() {
  ...
  return (
    <div className='max-w-xl'>
      <Link href='/projects' className='text-sm hover:underline mb-4 inline-block' style={{ color: 'var(--foreground)' }}>← 返回</Link>
      <h1 className='text-2xl font-bold mb-6' style={{ color: 'var(--foreground)' }}>编辑项目</h1>
      <form onSubmit={handleSubmit} className='space-y-4'>
        <div>
          <label className='block text-sm mb-1' style={{ color: 'var(--foreground)' }}>项目名称 *</label>
          <input type='text' value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className='w-full px-3 py-2 border rounded-md' style={{ backgroundColor: 'var(--background)', borderColor: 'var(--border)', color: 'var(--foreground)' }} required />
        </div>
        <div>
          <label className='block text-sm mb-1' style={{ color: 'var(--foreground)' }}>项目描述</label>
          <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className='w-full px-3 py-2 border rounded-md' style={{ backgroundColor: 'var(--background)', borderColor: 'var(--border)', color: 'var(--foreground)' }} rows={3} />
        </div>
        <div className='grid grid-cols-2 gap-4'>
          <div>
            <label className='block text-sm mb-1' style={{ color: 'var(--foreground)' }}>GitHub</label>
            <input type='url' value={form.git_url} onChange={(e) => setForm({ ...form, git_url: e.target.value })} className='w-full px-3 py-2 border rounded-md' style={{ backgroundColor: 'var(--background)', borderColor: 'var(--border)', color: 'var(--foreground)' }} />
          </div>
          <div>
            <label className='block text-sm mb-1' style={{ color: 'var(--foreground)' }}>在线地址</label>
            <input type='url' value={form.live_url} onChange={(e) => setForm({ ...form, live_url: e.target.value })} className='w-full px-3 py-2 border rounded-md' style={{ backgroundColor: 'var(--background)', borderColor: 'var(--border)', color: 'var(--foreground)' }} />
          </div>
        </div>
        <div>
